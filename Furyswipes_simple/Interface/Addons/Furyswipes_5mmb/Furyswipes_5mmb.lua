FSMB_toonlist={"Thelnsane","Elonlnside","tolnfinity","Thelraq","Anlmpulse"}
FSMB_invitelist={"Thelnsane","Elonlnside","tolnfinity","Thelraq","Anlmpulse"}
...
FSMB_version="120723_simple_retail"
FSMB_game="shadow"
FSMB_RAID = "MULTIBOX_myraid1"
AceComm=LibStub("AceComm-3.0")
print('Hello from Furyswipes and 5-Minute Multiboxing!')
...
FSMB_meleelist={}
FSMB_tank="Thelnsane"
FSMB_healerlist={"Anlmpulse"}
FSMB_clothto="Elonlnside"
...
function init()
	...
	if FSMB_game=="shadow" then
		...
		elseif myClass=="PALADIN" then
			if myspec==1 then myspec="HOLY" FSMB_IsMelee=true
	...
	if not FSMB_nomacros then
		...
		elseif FSMB_game=="shadow" then
			index=CreateMacroFS("setup_fs","Spell_magic_polymorphchicken","#showtooltip\n/click [button:1] "..myspec.."_SETUP LeftButton t;"..myspec.."_SETUP",nil)
		...
		ClearCursor()
		if (FSMB_game=="shadow") then
			index=CreateMacroFS("single_fs","ability_searingarrow","#showtooltip\n/click [button:1] "..myspec.."_SINGLE LeftButton t;"..myspec.."_SINGLE",nil)
		elseif (FSMB_game=="classic") then
			if myname==FSMB_tank then
		...
		if (FSMB_game=="shadow") then
			index=CreateMacroFS("multi_fs","ability_upgrademoonglaive","#showtooltip\n/click [button:1] "..myspec.."_MULTI LeftButton t;"..myspec.."_MULTI",nil)
		elseif (FSMB_game=="classic") then
			if myname==FSMB_tank then
		...
		if FSMB_game=="shadow" or FSMB_game=="classic" then
			index=CreateMacroFS("turbo_fs","spell_nature_lightning","#showtooltip\n/click [button:1] "..myspec.."_TURBO LeftButton t;"..myspec.."_TURBO",nil)
		else
		...
		ClearCursor()
		if FSMB_game=="shadow" then
			index=CreateMacroFS("aoe_fs","spell_fire_selfdestruct","#showtooltip\n/click [button:1] "..myspec.."_AOE LeftButton t;"..myspec.."_AOE",nil)
		...
		ClearCursor()
		index=CreateMacroFS("sheal_fs","spell_nature_healingtouch","#showtooltip\n/click [button:1] "..myspec.."_SHEAL LeftButton t;"..myspec.."_SHEAL",nil)
		PickupMacro(index)
		PlaceAction(6)
		if myClass=="DRUID" or FSMB_game~="shadow" then
			PickupMacro(index)
